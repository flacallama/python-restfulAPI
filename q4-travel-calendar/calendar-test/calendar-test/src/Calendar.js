...
class Calendar extends Component {
  ...
  selectDate = (e, index) => {
    if(!this.state.selected.includes(e)){
      this.setState({
        selected: this.state.selected.concat(e)
      })
    } else {
      this.setState({
        selected: this.state.selected.filter(x => x != e)
      })
    }
  ...
  render () {
    ...
    .map((elem, i)=> <DateBox key={i} index={i} element={elem} selectDate={this.selectDate} />
    )
